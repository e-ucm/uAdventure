...
$arguments = "-batchmode -force-free -username $Evn:license_username -password $Evn:license_password -nographics -silent-crashes -logFile $($log_file) -projectPath $($project_path) -quit -executeMethod $($fix_guids_method) $($guid_file) $($dll_file)"
